...
import type { ProfileUpdateInput } from "@/types/entities"
import { useDispatch, useSelector } from "react-redux"
import { fetchDoctors, reset as doctorReset } from "@/redux/features/doctor-slice"
import { getMyDetails, updateProfile, reset } from "@/redux/features/user-slice"
  ...
  const renderField = (label: string, field: keyof ProfileUpdateInput, type: FormFieldType) => {
    ...
    const renderEditMode = () => {
      ...
      switch (type) {
        ...
        default:
          fieldContent = (
            <CustomFormField fieldType={type} control={control} name={field} label={label} placeholder={label} />
          )
      }
    ...
    const renderViewMode = () => (
      <>
        ...
        <Button variant="ghost" size="icon" className="absolute top-0 right-0" onClick={() => handleEdit(field)}>
          <Pencil className="h-4 w-4" />
        </Button>
      </>
    )

    return <div className="relative">{editingField === field ? renderEditMode() : renderViewMode()}</div>
  }
    ...
    <Form {...form}>
      <form onSubmit={handleSubmit(onSave)} className="text-gray-800 space-y-5 p-4">
        ...
        <div className="flex justify-end space-x-4 mt-6">
          ...
          <SubmitButton className="shad-primary-btn" isLoading={isLoading}>
            Save Changes
          </SubmitButton>
        </div>
      </form>
